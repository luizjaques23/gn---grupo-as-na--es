                ...
                {step === 1 && (
                  <motion.div key="s1" {...stepAnim}>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-5 bg-paper-raised border-t border-l border-rule-soft">
                      ...
                      <div
                        className="cat-tile select-none border-b border-r border-rule-soft relative"
                        style={{ ['--tint' as string]: '#DC2626' }}
                      >
                        <span className="cat-tile__band" style={{ background: '#DC2626' }} aria-hidden />
                        <span className="block px-4 pt-5 pb-6">
                          <span className="flex items-center gap-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-red-600 animate-pulse" />
                            <span className="legend-strong text-[10px] block" style={{ color: '#DC2626' }}>JOVEM</span>
                          </span>
                          <span className="text-[0.7rem] text-ink-3 block mt-2">Jovens</span>
                          <span className="mt-5 inline-block text-[10px] font-extrabold uppercase tracking-widest text-red-600 dark:text-red-400 animate-blink">
                            Em breve
                          </span>
                        </span>
                      </div>
...
function Credit({ name, email }: { name: string; email: string | null }) {
  if (!email) {
    return (
      <span className="legend-strong text-[11px] sm:text-xs" style={{ color: 'var(--seal)' }}>
        {name}
      </span>
    );
  }
  return (
    <a
      href={`mailto:${email}`}
      className="group inline-flex items-center gap-2 legend-strong text-[11px] sm:text-xs px-2.5 py-1 rounded-sm border border-rule hover:border-seal hover:bg-paper-sunk transition-all"
      style={{ color: 'var(--seal)' }}
      title={`Escrever para ${name} — ${email}`}
    >
      <Mail className="w-3.5 h-3.5 text-seal group-hover:scale-110 transition-transform" />
